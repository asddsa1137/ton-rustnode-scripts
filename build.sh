...
sed -i -e '/^\[dependencies\]/p; s/\[dependencies\]/ed25519-dalek = "1.0"/' "${NODE_BUILD_DIR}/ton-node/Cargo.toml"
sed -i -e 's%features = \[\"cmake_build\", \"dynamic_linking\"\]%features = \[\"cmake_build\"\]%g' "${NODE_BUILD_DIR}/ton-node/Cargo.toml"
sed -i -e '/^\[features\]/p; s/\[features\]/sha2-native = ["sha2\/asm", "ed25519-dalek\/asm"]/' "${NODE_BUILD_DIR}/ton-node/Cargo.toml"
